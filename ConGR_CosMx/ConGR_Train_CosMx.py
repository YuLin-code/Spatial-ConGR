...
from CL_data_preprocess import load_data_cosmx, generate_coords_use_exp_cosmx, load_image_data_cosmx, image_data_prepare_tile_cosmx, normalization_min_max
from util_function import Validate_CL_Embedding_COSMX
import random
...
def parse_args():
    ...
    # original
    parser.add_argument('-sample', type=str, nargs='+', default=['fov1'], help='which sample to generate: fov1-fov30.')
...
if __name__ == "__main__":
    ...
    #os
    emb_path = '../Dataset/Human_Lung/gene_embedding/'
    img_path = '../Dataset/Human_Lung/image_data/'
    meta_path = '../Dataset/Human_Lung/meta_data/'
    ...
    #load spatial data
    adata = load_data_cosmx(emb_path, meta_path, transform_opt, sample)

    #spatial data preprocessing
    coords, use_expression_np = generate_coords_use_exp_cosmx(adata,select_gene_num)
    ...
    #load image data 
    img_3d, cell_diameter_radius, cell_row_in_fullres, cell_col_in_fullres = load_image_data_cosmx(img_path, sample, adata)
    print('cell_row_in_fullres',cell_row_in_fullres)
    print('cell_col_in_fullres',cell_col_in_fullres)
    print('cell_diameter_radius',cell_diameter_radius)
    
    #image data preprocessing
    adata_loc = adata.obs.index
    cropped_img_3d_nor = image_data_prepare_tile_cosmx(img_3d, cell_diameter_radius, cell_row_in_fullres, cell_col_in_fullres, adata_loc, tile_path)
    ...
    Validate_CL_Embedding_COSMX(model_epoch, learning_name, sample, adata, embedding_path, meta_path, cluster_result_path, validate_model_epoch)
