...
def load_image_data(image_path, sample, adata):
...
#-------------------------------------HE data preprocess------------------------------------------#
def he_data_prepare(he_img_3d, radius, spot_row_in_fullres, spot_col_in_fullres):
    #generate 3d HE data for resnet50
    init_3d_resnet = np.ones(shape=(len(spot_row_in_fullres),radius*2,radius*2,3), dtype=np.uint8)
    for crop_index in range(len(spot_row_in_fullres)):
        cropped_3d_resnet_current = he_img_3d[(spot_row_in_fullres[crop_index]-radius):(spot_row_in_fullres[crop_index]+radius),(spot_col_in_fullres[crop_index]-radius):(spot_col_in_fullres[crop_index]+radius),:]
        init_3d_resnet[crop_index,:,:,:] = cropped_3d_resnet_current
        print('cropped_3d_resnet_current is', cropped_3d_resnet_current)
        print('cropped_3d_resnet_current shape is', cropped_3d_resnet_current.shape)
        print('cropped_3d_resnet_current max is', np.max(cropped_3d_resnet_current))
    init_3d_resnet = init_3d_resnet.transpose((0,3,1,2))
    cropped_3d_resnet_nor = init_3d_resnet/255
    #print('cropped_3d_resnet_nor max is ',np.max(cropped_3d_resnet_nor))      #test
    #print('cropped_3d_resnet_nor shape is',cropped_3d_resnet_nor.shape)      #test
    return cropped_3d_resnet_nor

#-------------------------------------HE data preprocess------------------------------------------#
def he_data_prepare_for_original(he_img_3d, radius, spot_row_in_fullres, spot_col_in_fullres):
    #generate 3d HE data for resnet50
    init_3d_resnet = np.ones(shape=(len(spot_row_in_fullres),radius*2*radius*2), dtype=np.uint8)
    for crop_index in range(len(spot_row_in_fullres)):
        cropped_3d_resnet_current = he_img_3d[(spot_row_in_fullres[crop_index]-radius):(spot_row_in_fullres[crop_index]+radius),(spot_col_in_fullres[crop_index]-radius):(spot_col_in_fullres[crop_index]+radius)].reshape(1,-1)
        init_3d_resnet[crop_index,:] = cropped_3d_resnet_current
    cropped_3d_resnet_nor = init_3d_resnet/255
    #print('cropped_3d_resnet_nor max is ',np.max(cropped_3d_resnet_nor))      #test
    #print('cropped_3d_resnet_nor shape is',cropped_3d_resnet_nor.shape)      #test
...
def image_data_prepare_tile(img_3d, crop_radius, spot_row_in_fullres, spot_col_in_fullres, patch_target_size, adata_barcode, tile_path):
    ...
    for crop_index in range(len(spot_row_in_fullres)):
        ...
        #tile_name = str(barcode) + '_' + str(crop_radius*2)
